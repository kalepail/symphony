        ...
        F: FnMut(CodexEvent),
    {
        ...
        match method {
            ...
            "item/tool/call" => {
                ...
                {
                    "tool_call_completed"
                } else if tool != dynamic_tool::LINEAR_GRAPHQL_TOOL {
...
while IFS= read -r line; do
  ...
  case "$count" in
    ...
    4)
      ...
      printf '%s\n' '{{"id":103,"method":"item/tool/call","params":{{"name":"linear_graphql","callId":"call-1","threadId":"thread-tool","turnId":"turn-tool","arguments":{{"query":"query Viewer {{ viewer {{ id }} }}"}}}}}}'
      ;;
    5)
      printf '%s\n' '{{"method":"turn/completed"}}'
      exit 0
...
"#,
        ...
        let app = AppServerClient::new(
            config,
            Arc::new(StubTracker {
                raw_result: Err(TrackerError::LinearApiRequest("boom".to_string())),
            }),
        );
        let mut session = app.start_session(&workspace).await.expect("session");
        let events = Arc::new(Mutex::new(Vec::<CodexEvent>::new()));
