...
- Optional web observability can be enabled via CLI `--port` or `server.port` in `WORKFLOW.md`. `server.host` is also supported; the default bind host remains loopback (`127.0.0.1`). The dashboard now uses live SSE updates, keeps runtime clocks moving client-side, and falls back to `/api/v1/state` polling if the stream is unavailable. The terminal dashboard is enabled independently through `observability.dashboard_enabled` or the legacy `observability.terminal_enabled` alias. When the snapshot path degrades, `/api/v1/state` returns a structured `503` JSON payload that preserves distinct `snapshot_timeout` and `snapshot_unavailable` error codes, and `/` returns a `503` HTML dashboard shell instead of switching to a raw JSON error page.
- Logs are written to `./log/symphony.log` relative to the current working directory. Common paths in this repo are:
  - launching from `rust-todoist/`: `rust-todoist/log/symphony.log`
  - launching from the repo root: `log/symphony.log`
  - launching from `elixir/`: `elixir/log/symphony.log`
  - overriding with `--logs-root /path/to/root`: `/path/to/root/log/symphony.log`
- Rust log rotation is size-based, not daily: Symphony writes to `symphony.log`, rotates before a write would exceed 10 MB, renames the old file to `symphony.log.1`, shifts older files through `.5`, and deletes the oldest archive beyond that cap.
- File logs include RFC3339 millisecond timestamps, and Symphony's own lifecycle targets remain at `info` even when the surrounding shell uses a stricter `RUST_LOG` value such as `warn`. Logging conventions, correlation keys, retry taxonomy, and operator triage rules are documented in [`rust-todoist/docs/logging.md`](./docs/logging.md).
